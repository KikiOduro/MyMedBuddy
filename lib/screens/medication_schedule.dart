...
class _MedicationScheduleScreenState extends State<MedicationScheduleScreen> {
  ...
  // Maps conditions to common medications
  ...
  void initState() {
    super.initState();
    _medicationsFuture = loadConditionAndFetchMedications(); // fetch meds on init
  }

  // Loads stored condition and fetches related meds from RxNav API
  Future<List<String>> loadConditionAndFetchMedications() async {
    ...
    for (String drug in drugs) {
      final url = Uri.parse('https://rxnav.nlm.nih.gov/REST/drugs.json?name=$drug');
    ...
    return fetchedMeds.isNotEmpty ? fetchedMeds : ['No detailed results found from RxNav API.'];
  }

  // Opens time picker and stores selected time for a medication
  ...
  // Saves medication schedule with selected time
  Future<void> _saveMedication(String medName) async {
    ...
    final medDateTime = DateTime(now.year, now.month, now.day, time.hour, time.minute);
